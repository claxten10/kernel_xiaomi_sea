...
static const char *panel_name = "panel_name=dsi_panel_k7sr_38_0c_0b_video";
...
//	lcm_dcs_write_seq_static(ctx, 0xB1, 0x27, 0x2D); /*Flat mode off*/
//	lcm_dcs_write_seq_static(ctx, 0xF0, 0xA5, 0xA5);
	...
	/*ACL mode*/
	//lcm_dcs_write_seq_static(ctx, 0x55, 0x00); //ACL off
	/* HBM mode on */
...
#endif
		pr_debug("%s-\n", __func__);
		//pr_err("[XMFP]:%s++++ BLANK = DRM_BLANK_UNBLANK ++++ \n", __func__);
...
static int panel_srgb_control(struct drm_panel *panel)
{
	struct mtk_ddic_dsi_msg *cmd_msg =
			vmalloc(sizeof(struct mtk_ddic_dsi_msg));
	int ret;
	char leavel2_key_on[] = {0xF0, 0x5A,0x5A};
	char flat_gamma1[] = {0xB0, 0x00, 0x88, 0xB1};
	char flat_gamma2[] = {0xB1, 0x27, 0x2D};
	char leavel2_key_off[] = {0xF0, 0xA5,0xA5};
	if (unlikely(!cmd_msg)) {
		pr_err("%s vmalloc mtk_ddic_dsi_msg is failed, return\n", __func__);
		return 0;
	}
	...
	void *handle, unsigned int level)
{
	...
	pr_debug("%s: K7Sr for backlight level = %d,1=%x,2=%x,3=%x\n", __func__, level,bl_tb0[0],bl_tb0[1],bl_tb0[2]);
	...
	unsigned int mode)
{
	...
	unsigned int i = 0;
	pr_debug("%s: K7SR for get_mode_by_id mode==%d\n", __func__,mode);
...
static int mtk_panel_ext_param_set(struct drm_panel *panel, unsigned int mode)
{
	...
	pr_debug("%s: K7SR for mtk_panel_ext_param_set\n", __func__);
...
{
	struct lcm *ctx = panel_to_lcm(panel);
	pr_debug("%s: K7SR for mode_switch_60_to_90\n", __func__);
...
{
	struct lcm *ctx = panel_to_lcm(panel);
	pr_debug("%s: K7SR for mode_switch_90_to_60\n", __func__);
...
{
	int ret = 0;
	pr_debug("%s: K7SR for mode_switch\n", __func__);
...
static void lcm_esd_restore_backlight(struct drm_panel *panel)
{
	...
	lcm_dcs_write_seq_static(ctx, 0x51, 0x03, 0xFF);
  	pr_debug(" add:lcm_esd_restore_backlight \n");
	return;
}
...
{
	pr_debug("%s: K7SR for change_drm_disp_mode_params\n", __func__);
...
{
	pr_debug("%s: K7SR for lcm_get_modes\n", __func__);
...
/*	if (need_fake_resolution) {
	...
	drm_mode_probed_add(panel->connector, mode); //==>default mode添加成功，且为mode1.
	...
	drm_mode_probed_add(panel->connector, mode2);//==>peformance mode添加成功，为mode2.
