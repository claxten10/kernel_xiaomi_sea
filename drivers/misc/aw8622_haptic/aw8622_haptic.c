...
static void aw8622_wavefrom_data_load(const struct firmware *cont, void *context)
{
	...
	p_waveform_data->sample_freq =  (cont->data[2]<<8) | (cont->data[3]<<0);
	...
	p_waveform_data->max_sample_val = (cont->data[0]<<8) | (cont->data[1]<<0);
...
static int aw8622_play_wave(struct aw8622_haptic *haptic)
{
	...
	if (p_waveform_info->max_sample_val) {
		haptic->h_l_period = HAPTIC_PWM_MEMORY_MODE_CLOCK/p_waveform_info->max_sample_val/p_waveform_info->sample_freq;
	}
...
static DEVICE_ATTR(info, S_IWUSR | S_IRUGO, aw8622_vendor_info_show,
            NULL);
